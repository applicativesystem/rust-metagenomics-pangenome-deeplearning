...
This part is reading the paf-alignments and using them for the direct import
...
fn main() {
    ...
    let paf_alignment = paf_alignments(&args.pafalignment).unwrap();
    println!("The pangenome statistics analyzer: {}", paf_alignment);
}

fn paf_alignments(pathpaf: &str) -> Result<String, Box<dyn Error>> {
